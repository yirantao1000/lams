"""
kinova_basics.py: Kinova Arm Control Module (modified from official Kinova Arm Control codes)

This module provides functions and classes to interface with and control a Kinova robotic arm. It includes functionalities for sending movement commands, handling inverse kinematics, and controlling the gripper. It leverages the Kinova Kortex API for robotic arm operations.

Key Features:
1. **Basic Control Commands**:
   - Functions for sending joint and twist commands for precise movement control.
   
2. **Cartesian and Angular Movements**:
   - High-level functions to move the arm to specific Cartesian positions or angular configurations.

3. **Gripper Control**:
   - Classes for both high-level and low-level gripper command handling, enabling precise positioning and speed control.

4. **Inverse Kinematics**:
   - A function to compute joint angles from a target Cartesian pose using Kinova's inverse kinematics.

5. **Event-Driven Movement Completion**:
   - Notifications and threading for handling asynchronous operation completion.
"""
...
def twist_command(base, target, mode = 'teleoperation'):
...
class GripperCommandExample:
    ...
    def SendGripperSpeed(self, open = True, speed : float = 0.1):
        gripper_command = Base_pb2.GripperCommand()
        finger = gripper_command.gripper.finger.add()
